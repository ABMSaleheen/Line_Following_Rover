...
class AutonmousLineFollowerNode : public rclcpp::Node {

public:
  AutonmousLineFollowerNode():Node("autonomous_driving_node") {
    // int codec = cv::VideoWriter::fourcc('M','J','P','G');
    cv::Size frame_size(640, 480);


    vid_subscriber = this->create_subscription<sensor_msgs::msg::Image>(
                  "/camera_scan_fwd/image_raw", 20, std::bind(&AutonmousLineFollowerNode::image_process_callback, 
    ...
    timer_vel_pub = this->create_wall_timer(std::chrono::microseconds(1000),
                std::bind(&AutonmousLineFollowerNode::send_cmd_vel, this));
...
int main(int argc, char *argv[]) {
  ...
  rclcpp::spin(std::make_shared<AutonmousLineFollowerNode>());
  rclcpp::shutdown();
  return 0;
}